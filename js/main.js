...
const mediaClips = [
    {
        src: 'videos/car-trading.mp4',
        title: 'Trading',
        description: 'Providing luxury vehicles and high-quality<br>industrial, construction, and safety materials.',
        link: 'trade'
    },
    {
        src: 'videos/realestate.mp4',
        title: 'Real Estate',
        description: 'Offering residential and commercial real estate<br>solutions, including property development, management, sales, and rental services.',
        link: 'realestate'
    },
    {
        src: 'videos/contracting.mp4',
        title: 'Contracting',
        description: 'Delivering civil and electromechanical works<br>for industrial, commercial, and infrastructure projects.',
        link: 'contracting'
    }
    // Additional clips...
];
...
const headingElement = document.querySelector('.info-overlay h3');
const textElement = document.querySelector('.info-overlay p');
const actionButton = document.querySelector('.cta-button');
const videoContainer = document.querySelector('.media-container');

// Load the initial video and update the overlay
updateMediaClip();

function updateMediaClip() {
    const currentClip = mediaClips[currentClipIndex];
    ...
    setTimeout(() => {
        // Update video source
        mediaPlayer.src = currentClip.src;
        mediaPlayer.load();
        mediaPlayer.play();
        
        // Update title, description, and learn more link
        headingElement.textContent = currentClip.title;
        textElement.innerHTML = currentClip.description; // Supports <br> tags
        actionButton.href = currentClip.link;
        
        
        // Fade-in title, then description, then button
        setTimeout(() => {
            headingElement.classList.remove('fade-out');
            headingElement.classList.add('fade-in');
        }, 400); // Delayed transition for title

        setTimeout(() => {
            textElement.classList.remove('fade-out');
            textElement.classList.add('fade-in');
        }, 800); // Delayed transition for description

        setTimeout(() => {
            actionButton.classList.remove('fade-out');
            actionButton.classList.add('fade-in');
        }, 1000); // Delayed transition for button

        videoContainer.classList.remove('fade-out');
        videoContainer.classList.add('fade-in');

    }, 500); // Wait for the fade-out transition to complete before updating
}


// Listen for the 'ended' event to play the next video
mediaPlayer.addEventListener('ended', () => {
    currentClipIndex = (currentClipIndex + 1) % mediaClips.length; // Move to the next video
    updateMediaClip(); // Update the video player
});

// Event listeners for next and previous buttons
document.getElementById('nextClipButton').addEventListener('click', () => {
    currentClipIndex = (currentClipIndex + 1) % mediaClips.length; // Move to the next video
    updateMediaClip(); // Update the video player
});

document.getElementById('prevClipButton').addEventListener('click', () => {
    currentClipIndex = (currentClipIndex - 1 + mediaClips.length) % mediaClips.length; // Move to the previous video
    updateMediaClip(); // Update the video player
});
...
const serviceCards = document.querySelectorAll('.mil-service-card-sm');

// Loop through each card
serviceCards.forEach(card => {
    const video = card.querySelector('.service-video');

    // Variable to track if video is playing
    let isPlaying = false;

    // Add event listeners for mouseenter and mouseleave (for desktop)
